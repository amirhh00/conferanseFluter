...
class Fdesktop extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.hash !== this.props.hash && nextProps.hash.includes("fdesktop")) {
      setTimeout(() => {
        // this.player.play();
  ...
  render() {
    return (
      <div className={this.state.animateClassNames} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%' }} >
        ...
        <div style={{ textAlign: 'right', marginTop: 20, direction: 'rtl' }} >
          <h4>فلاتر در Desktop</h4>
        </div>
        <img style={{ width: '100%' }} src="images/fDesktop.png" />
      </div>
    )
  }
}
...
export default connect(mapStateToProps)(withRouter(Fdesktop));
