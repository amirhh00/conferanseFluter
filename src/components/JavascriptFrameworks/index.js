...
class JSFramWorks extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.hash !== this.props.hash && nextProps.hash.includes("jsFrameWorks")) {
      setTimeout(() => {
        // this.player.play();
    ...
    return (
      <div className={this.state.animateClassNames} style={{display:'flex',flexDirection:'column',alignItems:'center',height:'100%'}} >
        ...
        <div style={{textAlign:'right',marginTop:20}} >
          <h4>فریمورک های جاوااسکریپتی</h4>
        </div><br />
        <img style={{width : '50%'}} src="images/reactNativeLogo.png" />
        <img style={{width : '50%'}} src="images/vueNativeLogo.png" />
        <img style={{width : '50%'}} src="images/nativeScriptLogo.png" />
      </div>
    )
  }
}
...
export default connect(mapStateToProps)(withRouter(JSFramWorks));
