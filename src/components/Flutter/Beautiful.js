...
class Beautiful extends Component {
  constructor(props) {
    super(props)
    this.state = {
      source: "images/beautiful1.mp4",
      animateClassNames : "fadein"
    }
  }

  componentWillReceiveProps(nextProps) {
    if (nextProps.hash !== this.props.hash && nextProps.hash.includes("beautiful")) {
      setTimeout(() => {
        this.player.play();
...
export default connect(mapStateToProps)(withRouter(Beautiful));
