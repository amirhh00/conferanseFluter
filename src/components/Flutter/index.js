...
class Flutter1 extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.hash !== this.props.hash && nextProps.hash.includes("flutter1")) {
      setTimeout(() => {
        // this.player.play();
  ...
  render() {
    return (
      <div className={this.state.animateClassNames} style={{display:'flex',flexDirection:'column',alignItems:'center',height:'100%'}} >
        ...
        <img style={{width : '100%'}} src="images/flutter_attrs.jpg" />
      </div>
    )
  }
}
...
export default connect(mapStateToProps)(withRouter(Flutter1));
