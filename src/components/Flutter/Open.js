...
class Open extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.hash !== this.props.hash && nextProps.hash.includes("open")) {
      setTimeout(() => {
        // this.player.play();
    ...
    return (
      <div className={this.state.animateClassNames} style={{display:'flex',flexDirection:'column',alignItems:'center',height:'100%'}} >
        ...
        <div style={{textAlign:'right',marginTop:20}} >
          <h4>فلاتر متن بازه</h4>
          <p style={{fontSize:'smaller'}} >BSD License / make widgets and extend core</p>
        </div><br />
        <img style={{width : '100%'}} src="images/rawe.jpg" />
      </div>
    )
  }
}
...
export default connect(mapStateToProps)(withRouter(Open));
